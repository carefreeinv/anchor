...
**The coordinator's context is the scarce resource this loop protects, and only the structured footer crosses back into it.** An executor's full reply — reasoning, false starts, quoted templates — never reaches the orchestrator's own context or the next task's prompt; `scripts/fleet_metrics.py`'s `extract_footer` pulls mythos-core rule 8's `## Result` / `## How to verify` / `## Deferred / concerns` out tolerantly (case/spacing drift, last-occurrence-wins when a model quotes the template while reasoning) and caps the result so a smuggled transcript can't ride along inside `## Result`. A reply with no recognizable footer at all is rejected back to the executor once with the expected shape quoted, then escalated per rule 6 — never silently accepted as unstructured output; `scripts/orchestrate.py` archives the full raw reply to `var/task-transcripts/<task-hash>.log` for post-mortem either way, discarded from the coordinator's context but not from disk.
