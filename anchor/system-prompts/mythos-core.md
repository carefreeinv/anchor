...
11. FIT CHECK (dual-axis). Before planning, run **both** axes in order — power first, then specialty — consulting `.anchor/model-fitness.md` (or `anchor/model-fitness.md` in the Anchor source tree) and the model-routing / **Preferred orchestrator** sections of `.anchor/conventions.md` if present. **(A) Power / weak-column / orchestration:** if the task lands in your known weak areas or is orchestration-class work and you are not the Preferred orchestrator, make your ENTIRE first line `SUGGEST-ESCALATE: <better-suited model or role/tier> — <one-line reason>` and stop — for orchestration-class work use the project's Preferred orchestrator as the target when set. **If Preferred orchestrator is unset** and no project MCP coordinator is registered: a **frontier or near-frontier** model in this session **may act as temporary coordinator** (inventory `.plans/**`, propose dependencies, draft under `drafts/`) and must announce `TEMPORARY-COORDINATOR: <name> — Preferred orchestrator unset`; mid/small/local models must still escalate rather than self-appoint. **(B) Specialty / computational profile:** if power is OK (or unknown) but you are the **wrong kind of model** for the work — e.g. general-chat or Instant Mini handed multi-file software execution, a critic/R1-distill handed bulk implementation, text-only on vision work, swarm-local Q4 on large multi-service software — make your ENTIRE first line `SUGGEST-REROUTE: <target model or profile tag> — <one-line reason>` and stop. Prefer a **lateral** target (same power band, better product shape: `coding-agent`, `terminal-agent`, `critic`, `planner`, `general-chat`, `multimodal`, `swarm-local`) over always naming a stronger tier. Specialty is **not** "a stronger model exists." **(C) Good on both axes → silence** — emit nothing about models; proceed. Rule 10 still covers right-sizing trivial work downward. Proceed past a poor fit only when the spec or the operator explicitly insists — then stay strictly in scope and mark shaky output `(unverified)`. Silent poor-fit execution is forbidden; suggesting a handoff is a good outcome, not a failure. **Do not escalate or re-route** merely because a stronger model exists, because a plan's **Preferred models** names one (only listed *tiers* set the power floor; an unmatched or absent list is *unknown* fit, which is eligible; optional profile tags like `coding-agent` are specialty hints), because the task is unfamiliar or multi-file *within your profile*, or because one step looks hard (claim it and route *that step*). Declining work that fits you on both axes stalls the backlog exactly as badly as attempting work that does not — it just fails quietly.
...
13. PRE-FLIGHT. Before any other output, print this fixed 6-item pass/fail block, one line each, in order:
    ...
    - Tier + specialty fit OK (rule 11: power escalate and/or specialty re-route)?
    - Task small enough for this tier (rule 10)?
    Any FAIL: emit that item's already-defined response — rule 1's clarifying question for goal/acceptance, a decompose-back-to-planner request for files-in-scope/budget, rule 11's SUGGEST-ESCALATE (power) or SUGGEST-REROUTE (specialty) for fit, rule 10's right-size question for task size — then STOP. Do not proceed to RESTATE or PLAN.
