...
**Field-tested choices for running capable models locally**: the Qwen3 family (especially 30B-A3B), Gemma 3, Mistral Small, DeepSeek-R1 distills, and Llama 3.3 — served via llama.cpp or vLLM, quantized Q4, official chat templates, short contexts.
...
2. Run `./config.sh` (or type `/config` in Claude Code / Grok Build) to pick your default platform(s), whether you want fleet tooling, and your model priority — the order you reach for / escalate between models (e.g. `nim,grok,openai:gpt-5,claude:sonnet,claude:opus,claude:fable`). It saves your answer and prints the exact `anchor <project-dir>` command to scaffold a project with it.
3. Scaffold a project: `anchor <project-dir>` (uses your saved defaults) or `anchor <project-dir> --platform claude,grok` (explicit, no defaults needed). See the [full CLI reference](tooling/cli) for `--fleet`, `--framework`, `--dry-run`, and drift-checking with `--check`.
4. Point `scripts/endpoints.yaml` at your endpoints; use `scripts/orchestrate.py` to run the orchestrator pattern across them — see [Hardware](hardware/h100) for serving models on your own fleet.
